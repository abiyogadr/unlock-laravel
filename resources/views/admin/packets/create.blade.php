...
@section('title', 'Unlock - Tambah Paket')
@section('page-title', 'Tambah Paket Webinar')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    {{-- Form Container --}}
    <form action="{{ route('admin.packets.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        @csrf

        {{-- Header Section --}}
        <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            <div>
                <h3 class="text-lg font-bold text-gray-800">Buat Paket Baru</h3>
                <p class="text-sm text-gray-500 mt-1">Tentukan detail paket dan persyaratannya.</p>
            </div>
            
            {{-- Status Toggle (Default: Aktif) --}}
            {{-- Logic Alpine: Cek old input, jika tidak ada default true (1) --}}
            <div class="flex items-center gap-3" x-data="{ active: {{ old('is_active', '1') == '1' ? 'true' : 'false' }} }">
        ...
        <div class="p-6 sm:p-8 space-y-8">
            ...
            <div class="col-span-3">
                ...
                <textarea name="description" id="editor" rows="6"
                          class="w-full px-4 py-3 rounded-lg border border-gray-300">{{ old('description') }}</textarea>
